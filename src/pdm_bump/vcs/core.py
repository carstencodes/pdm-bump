...
from typing import Optional, Dict, Tuple, Union, AnyStr, NamedTuple, Callable, Type, cast
...
class VcsProviderFactory(ABC):
    ...
    def _is_valid_root(self, path: Path) -> bool:
        ...
        is_valid_root_by_file: bool = fs_root.file_name is None or self._file_exists(path, fs_root.file_name)
        is_valid_root_by_dir: bool = fs_root.dir_name is None or self._dir_exists(path, fs_root.dir_name)
    ...
    def register(self, name: str) -> Callable:
        def decorator(clazz: Type):
            if not issubclass(clazz, VcsProviderFactory):
                raise ValueError(f"{clazz.__name__} is not an sub-type of {VcsProviderFactory.__name__}")
